...
class ConsumeController extends Controller{
    ...
    public function actionCount(){
        ...
        //查询总条数
        $count=\Yii::$app->db->createCommand(" select count(*) as totalCount from (select count(*) as sellCount,sum(deduction) as sellMoney,book_id from consume WHERE 1=1 $where group by book_id) as sell_tj,book where book.id = sell_tj.book_id")->queryScalar();
        $pager=new Pagination([
            'totalCount'=>$count,//总条数
            'defaultPageSize'=>20,//每页显示条数
        ]);
